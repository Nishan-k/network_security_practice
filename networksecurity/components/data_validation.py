...
class DataValidation:
    ...
    #--------------------
    def validate_columns_and_dtypes(self, df: pd.DataFrame) -> (bool, list):
        errors = []
        ...
        for col, dtype in self._schema_config["columns"].items():
            if col in df.columns:
                if not np.issubdtype(df[col].dtype, np.dtype(dtype).type):
                    errors.append(f"Column {col} expected data type: {dtype}, found: {df[col].dtype}")
        return len(errors) == 0, errors
    

    def validate_missing_values(self, df:pd.DataFrame, threshold: float = None) -> (bool, list):
        errors = []
        threshold = self._schema_config["missing_values"]["threshold"]
        for col in df.columns:
            missing_pct = df[col].isna().mean()
            if missing_pct > threshold:
                errors.append(f"The column: {col} has {missing_pct * 100:.2f}% missing values > allowed ({threshold * 100:.2f}%.)")
        return len(errors) == 0, errors
    ...
    def detect_data_drift(self, base_df: pd.DataFrame, current_df: pd.DataFrame, threshold: float = None) -> bool:
        ...
        for col in base_df.columns:
            ...
            drift_report[col] = {
                ...
                "drift_status": bool(pvalue < threshold)
            }
    ...
    def validate_dataframe(self, df: pd.DataFrame, data_name: str) -> (bool, list):
        is_valid, errors = self.validate_columns_and_dtypes(df)
        if not is_valid:
            errors = [f"[{data_name}] {err}" for err in errors]
            return False, errors
        
        for check in [self.validate_missing_values, self.validate_duplicates]:
            valid, errs = check(df)
            if not valid:
